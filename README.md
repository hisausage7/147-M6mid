    ...
    <div id="container">
      ...
      <div id="welcome">
        ...
        <input
          type="number"
          id="questionLimit"
          placeholder="輸入題數,至多100題 / Enter number of questions"
          style="width: 100%; padding: 8px; margin-bottom: 10px; font-size: 1em"
        />
        <input
          type="number"
      ...
      <div id="wrongReview" class="hidden">
        ...
        <div class="panel">
          <div class="muted" style="margin-bottom:10px;">
            列出「仍然答錯過」的題目。你之後做對同一題，會自動從清單移除。
          </div>

          <div class="controls" style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;justify-content:center;">
            <input id="wrongNameInput" type="text" placeholder="姓名"
                   style="flex:1;min-width:240px;padding:8px;font-size:1em;" />
            <input id="wrongQuestionLimit" type="number" placeholder="錯題測驗題數"
                   style="flex:1;min-width:240px;padding:8px;font-size:1em;" />
            <input id="wrongDurationInput" type="number" value="80" min="1" max="999"
                   placeholder="錯題測驗時間"
                   style="flex:1;min-width:240px;padding:8px;font-size:1em;" />
          </div>
    ...
    answer: "A"
        ...
        function renderWrongList() {
          ...
          if (!wrongQs.length) {
            ...
            tr.innerHTML = '<td class="mono">-</td><td colspan="4">目前沒有錯題記憶 🎉</td>';
            tb.appendChild(tr);
            return;
          }
        ...
        function showView(idToShow) {
          ["welcome", "quiz", "results", "bank", "wrongReview"].forEach((id) => {
        ...
        const openWrongPanelFromWelcome = document.getElementById("openWrongPanelFromWelcome");
        const openWrongPanelFromResults = document.getElementById("openWrongPanelFromResults");

        const wrongNameInput = document.getElementById("wrongNameInput");
        const wrongQuestionLimit = document.getElementById("wrongQuestionLimit");
        const wrongDurationInput = document.getElementById("wrongDurationInput");
        const startWrongQuizBtn = document.getElementById("startWrongQuizBtn");
        const clearWrongMemBtn = document.getElementById("clearWrongMemBtn");
        const backToWelcomeFromWrong = document.getElementById("backToWelcomeFromWrong");
        const toBankFromWrong = document.getElementById("toBankFromWrong");
        ...
        function openWrongPanel() {
          if (quizActive) {
            alert("測驗進行中不可進入錯題記憶畫面。請先完成或離開考試。");
            return;
          }

          // 以「錯題畫面姓名」優先，其次首頁姓名，最後 _guest
          var n = safeName((wrongNameInput && wrongNameInput.value) ? wrongNameInput.value : (nameInput ? nameInput.value : ""));
          if (!n || n === "_guest") {
            n = safeName(nameInput ? nameInput.value : "");
          }
          n = safeName(n);
        ...
        startBtn.addEventListener("click", () => {
          ...
          const qLimit = parseInt((questionLimitInput.value || "").trim(), 10);
          ...
          let minutes = parseInt((durationInput && durationInput.value) ? durationInput.value : "80", 10);
        ...
        /* ====== 錯題畫面：按鈕 ====== */
        if (backToWelcomeFromWrong) backToWelcomeFromWrong.addEventListener("click", function () {
          showView("welcome");
        });

        if (toBankFromWrong) toBankFromWrong.addEventListener("click", function () {
          enterBank();
        });

        if (wrongNameInput) wrongNameInput.addEventListener("input", function () {
          // 即時更新顯示（不存）
          currentUserName = safeName(wrongNameInput.value);
          wrongSet = loadWrongSet(currentUserName);
          updateWrongMemInfo();
          renderWrongList();
        });

        if (clearWrongMemBtn) clearWrongMemBtn.addEventListener("click", function () {
        ...
        if (startWrongQuizBtn) startWrongQuizBtn.addEventListener("click", function () {
          if (quizActive) {
            alert("測驗進行中不可開始錯題測驗。");
            return;
          }
        ...
        function renderBank() {
          bankBody.innerHTML = "";
          const sel = perPageSel.value;
          const per = sel === "all" ? bankFiltered.length : parseInt(sel || "10", 10);
          ...
          items.forEach((q, idx) => {
            ...
            const optsHtml = q.options
              .map((o) => {
                ...
                return `<span class="opt-row ${isC ? "is-correct" : ""}">${
        ...
        function showQ() {
          ...
          optionsWithFlag.forEach((opt) => {
            ...
            rd.onchange = function () {
              var correctText = q.options.find(function (optItem) {
                return optItem.charAt(0) === q.answer;
              });

              // 錯題記憶：答錯存起來；答對移除（表示這題你已經會了）
        ...
        /* ====== 初始化：用首頁姓名（若空則 _guest）顯示錯題數 ====== */
        currentUserName = safeName(nameInput ? nameInput.value : "");
        wrongSet = loadWrongSet(currentUserName);
        updateWrongMemInfo();
      });
    </script>
